...
local function walkToTarget(targetPosition)
    ...
    -- Pathfinding agent configuration
    ...
    if path.Status == Enum.PathStatus.Success then
        ...
        for _, waypoint in ipairs(waypoints) do
            -- Stop movement if character dies
            ...
            -- Timeout mechanism to prevent freezing (max 2 seconds per waypoint)
            local reached = false
            local connection
            ...
            while not reached and timeout < 2 do
                ...
                -- Pause auto-navigation if player moves manually
